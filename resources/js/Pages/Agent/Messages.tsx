...
type ActiveConv = { id: number; title: string; initials: string; type: string } | null;
...
export default function AgentMessages({ agent, conversations, active, messages }: Props) {
    ...
    function send() {
        ...
        setSending(true);
        router.post(`/agent/messages/${active.id}`, { body }, {
            preserveScroll: true,
            onSuccess: () => setBody(''),
    ...
    return (
        <AgentLayout crumb="Messages" agencyName={agent.agency_name}>
            <Head title="Messages" />
                ...
                <div className="w-72 shrink-0 bg-white border-r border-ink-200 flex flex-col">
                    <div className="p-4 border-b border-ink-200">
                        ...
                        <div className="relative">
                            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-ink-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                                <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
                            </svg>
                            <input
                                className="w-full bg-ink-50 border border-ink-200 rounded-lg pl-9 pr-3 py-1.5 text-[12px] focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand"
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {conversations.length === 0 ? (
                            ...
                            conversations.map((conv) => (
                                <button
                                    key={conv.id}
                                    onClick={() => router.get('/agent/messages', { conversation_id: conv.id }, { preserveState: true })}
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center justify-between mt-0.5">
                                                <p className="text-[11px] text-ink-500 truncate flex-1">{conv.preview}</p>
                                                {conv.unread_count > 0 && (
                                                    <span className="ml-1 w-4 h-4 rounded-full bg-brand-500 text-white text-[9px] font-bold flex items-center justify-center shrink-0">
                                                        {conv.unread_count}
                                                    </span>
                                                )}
                                            </div>
                    ...
                    <div className="flex-1 flex flex-col min-w-0">
                        ...
                        <div className="h-14 border-b border-ink-200 bg-white flex items-center px-5 gap-3 shrink-0">
                            ...
                            <div>
                                ...
                                <p className="text-[11px] text-ink-500 capitalize">{active.type?.replace('_', ' ')}</p>
                            </div>
                        </div>
