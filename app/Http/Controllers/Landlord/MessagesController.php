...
class MessagesController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $allIds = $conversations->flatMap(fn ($c) => $c['other_ids'])->unique()->values();
        $users  = User::whereIn('id', $allIds)->get(['id', 'name', 'role'])->keyBy('id');
        ...
        if ($activeId) {
            ...
            if ($conv) {
                ...
                foreach ($grouped as $day => $dayMessages) {
                    $messages[] = [
                        'type'  => 'date_divider',
                        'label' => Carbon::parse($day)->isToday() ? 'Today' : Carbon::parse($day)->format('d M Y'),
                    ];
        ...
        return Inertia::render('Landlord/Messages', [
            'landlord'      => ['id' => $user->id, 'name' => $user->name],
